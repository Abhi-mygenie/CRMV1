...
async def get_dashboard_stats(user: dict = Depends(get_current_user)):
    ...
    loyalty_orders_percent_30d = round((loyalty_orders_30d / total_orders_30d * 100), 1) if total_orders_30d > 0 else 0.0
    ...
    loyalty_orders_percent_7d = round((loyalty_orders_7d / total_orders_7d * 100), 1) if total_orders_7d > 0 else 0.0
    
    # === REVENUE SPLIT (Total, 30D, 7D) ===
    # Total Revenue Split
    ...
    # 30D Revenue Split
    repeat_revenue_30d_pipeline = [
        {"$match": {"user_id": user_id, "customer_id": {"$in": repeat_customer_ids}, "created_at": {"$gte": thirty_days_ago}}},
        {"$group": {"_id": None, "total": {"$sum": "$order_amount"}}}
    ]
    repeat_revenue_30d_result = await db.orders.aggregate(repeat_revenue_30d_pipeline).to_list(1) if repeat_customer_ids else []
    repeat_revenue_30d = repeat_revenue_30d_result[0].get("total", 0) if repeat_revenue_30d_result else 0
    
    new_revenue_30d_pipeline = [
        {"$match": {"user_id": user_id, "customer_id": {"$in": new_customer_ids}, "created_at": {"$gte": thirty_days_ago}}},
        {"$group": {"_id": None, "total": {"$sum": "$order_amount"}}}
    ]
    new_revenue_30d_result = await db.orders.aggregate(new_revenue_30d_pipeline).to_list(1) if new_customer_ids else []
    new_revenue_30d = new_revenue_30d_result[0].get("total", 0) if new_revenue_30d_result else 0
    
    total_revenue_30d_for_percent = repeat_revenue_30d + new_revenue_30d
    repeat_revenue_percent_30d = round((repeat_revenue_30d / total_revenue_30d_for_percent * 100), 1) if total_revenue_30d_for_percent > 0 else 0.0
    new_revenue_percent_30d = round((new_revenue_30d / total_revenue_30d_for_percent * 100), 1) if total_revenue_30d_for_percent > 0 else 0.0
    
    # 7D Revenue Split
    repeat_revenue_7d_pipeline = [
        {"$match": {"user_id": user_id, "customer_id": {"$in": repeat_customer_ids}, "created_at": {"$gte": seven_days_ago}}},
        {"$group": {"_id": None, "total": {"$sum": "$order_amount"}}}
    ]
    repeat_revenue_7d_result = await db.orders.aggregate(repeat_revenue_7d_pipeline).to_list(1) if repeat_customer_ids else []
    repeat_revenue_7d = repeat_revenue_7d_result[0].get("total", 0) if repeat_revenue_7d_result else 0
    
    new_revenue_7d_pipeline = [
        {"$match": {"user_id": user_id, "customer_id": {"$in": new_customer_ids}, "created_at": {"$gte": seven_days_ago}}},
        {"$group": {"_id": None, "total": {"$sum": "$order_amount"}}}
    ]
    new_revenue_7d_result = await db.orders.aggregate(new_revenue_7d_pipeline).to_list(1) if new_customer_ids else []
    new_revenue_7d = new_revenue_7d_result[0].get("total", 0) if new_revenue_7d_result else 0
    
    total_revenue_7d_for_percent = repeat_revenue_7d + new_revenue_7d
    repeat_revenue_percent_7d = round((repeat_revenue_7d / total_revenue_7d_for_percent * 100), 1) if total_revenue_7d_for_percent > 0 else 0.0
    new_revenue_percent_7d = round((new_revenue_7d / total_revenue_7d_for_percent * 100), 1) if total_revenue_7d_for_percent > 0 else 0.0
    ...
    return DashboardStats(
        ...
        loyalty_orders_percent_30d=loyalty_orders_percent_30d,
        loyalty_orders_percent_7d=loyalty_orders_percent_7d,
        repeat_revenue_percent=repeat_revenue_percent,
        new_revenue_percent=new_revenue_percent,
        repeat_revenue_percent_30d=repeat_revenue_percent_30d,
        new_revenue_percent_30d=new_revenue_percent_30d,
        repeat_revenue_percent_7d=repeat_revenue_percent_7d,
        new_revenue_percent_7d=new_revenue_percent_7d,
